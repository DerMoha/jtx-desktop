...
class TaskRepository(private val local: LocalDataSource) {
    ...
    fun getAllCombined(includeArchived: Boolean = false): Flow<List<CombinedEntry>> = local.getAllTasks(includeArchived).map { tasks ->
        tasks.map { task ->
            CombinedEntry(
                ...
                relatedEntries = task.relatedEntries,
    ...
    suspend fun updateTask(combined: CombinedEntry) {
        ...
        val updatedEntry = existing.copy(
            ...
            descriptionFormat = combined.descriptionFormat,
            due = combined.date,
            start = combined.startDate,
            progress = combined.progress ?: 0,
            completed = combined.completed ?: false,
            categories = combined.categories,
            color = combined.color,
            location = combined.location,
            subtasks = combined.subtasks,
            relatedEntries = combined.relatedEntries,
            recurrenceRule = combined.recurrenceRule,
            recurrenceDates = combined.recurrenceDates,
            exceptionDates = combined.exceptionDates,
            recurrenceId = combined.recurrenceId,
